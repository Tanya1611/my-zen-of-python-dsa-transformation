''' Last Occurrence of Element in an Unsorted Array

Given an array of N integers and an integer T. Design an algorithm to find the last occurrence of T in the array.
Note: Output -1 if T is not present in the array.

Example:
Input: n = 5, arr = [1, 2, 3, 2, 5], T = 2
Output: 3
Explanation: The last occurrence of 2 is at index 3.

'''

# Approach: Iterate array -> Match the target value -> Return index => Linear Search

def last_occurrence(arr: list[int], n: int, t: int) -> int:
    for i in range(n-1, -1, -1):
        if arr[i] == t:
            return i
    return -1

N = int(input("Enter the number of elements in the array: "))
arr = [int(input(f"Enter element of index {i}: ")) for i in range(N)]
T = int(input("Enter the target value to find: "))

print(f"The last occurrence of {T} is at index: {last_occurrence(arr, N, T)}")


# Output:
# Enter the number of elements in the array: 5
# Enter element of index 0: 10
# Enter element of index 1: 30
# Enter element of index 2: 50
# Enter element of index 3: 20
# Enter element of index 4: 50
# Enter the target value to find: 50
# The last occurrence of 50 is at index: 4